...
def compare_results(file1, file2, *files):
    ...
    for indicator in indicators:
        ...
        results[indicator] = ([mean1, std1, min1, max1], [mean2, std2, min2, max2])
        print(
            f"{indicator} 1: mean={mean1:.2f}, std={std1:.2f}, min={min1:.2f}, max={max1:.2f}"
        )
        print(
            f"{indicator} 2: mean={mean2:.2f}, std={std2:.2f}, min={min2:.2f}, max={max2:.2f}"
        )
    ...
    for indicator in indicators:
        ...
        print(
            f"| {file1} | {indicator} | {mean1:.2f} | {std1:.2f} | {min1:.2f} | {max1:.2f} |"
        )
        print(
            f"| {file2} | {indicator} | {mean2:.2f} | {std2:.2f} | {min2:.2f} | {max2:.2f} |"
        )
